...
public class Discotienda
{
    ...
    //crear el archivo con la clase File	
    	...
    	String generoDadoPorElUsuario= JOptionPane.showInputDialog("digite el genero");
    	...
    	for (int i=0 ;i<discos.size(); i++)
    	{
    		...
    		//escribir.con la pluma la informacion requerida
    		if (miDisco.darGenero().equals(generoDadoPorElUsuario)) 
    		{
    	...
    	if(generoDiscoEncontrado >0 && precioDiscoEncontrado>0) {
    		...
    		JOptionPane.showMessageDialog(null,"error el disco no cuenta con los requisitos necesarios");
    ...
    public String metodo2( )
    {
    	try {
		DiscosCostosos();
			return "felicitaciones reporte generado ";
		} catch (FileNotFoundException e) {
			// devolver el posible error
